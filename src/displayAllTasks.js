...
const disiplayOnDOM = (indexDisplay, i) => {
  ...
  eachToDoHolder.innerHTML += ` <div class="left-task-items">
  ...
  <div class="done"><button class="check-stat">${indexDisplay.getCheckStat()}</button></div>
...
const styleCheckButton = (item) => {
  const buttonDisplay = document.querySelectorAll(item);
  buttonDisplay.forEach((btn) => {
    if (btn.textContent === "true") {
      btn.textContent = "Done";
    } else {
      btn.textContent = "Not Done yet";
    }
  });
};
